(function () {
    ...
    angular.module('app')
        .controller('DashboardCtrl', ['$scope', 'globalService', DashboardCtrl])

    function DashboardCtrl($scope, globalService) {
        var init;

        $scope.dashboard = {};
        $scope.dashboard.history = [];
        $scope.search = search;
        ...
        function search() {
            globalService.list('dashboard').then(function(response){
                $scope.dashboard = response.data;
            });

            globalService.list('dashboard/history').then(function(response){
                $scope.dashboard.history = response.data.map(function(item){
                    if(item){
                        if((item['movie'] !== undefined) && (item['user'] !== undefined)){

                            if((item['movie']['_id'] !== undefined) && (item['user']['_id'] !== undefined)){
                                item.movie.progress = 50;
                                return item;

                                /*globalService.find('movie/progress', {user: item.user._id, movie: item.movie._id}).then(function(response){
                                    return item;
                                });*/
                            }
